...
class Modularity(object):
    def __init__(self,G):
        ...
        Orig_deg = {}
        Orig_deg = self.UpdateDeg(A, G.nodes())

        #run Newman alg
        self.runGirvanNewman(G, Orig_deg, m_)

   
    #this method just reads the graph structure from the file
    def buildG(self,G, file_, delimiter_):
    ...
    def CmtyGirvanNewmanStep(self,G):
    ...
    def _GirvanNewmanGetModularity(self,G, deg_, m_):
        New_A = nx.adj_matrix(G)
        New_deg = {}
        New_deg = self.UpdateDeg(New_A, G.nodes())
        #Let's compute the Q
    ...
    def runGirvanNewman(self,G, Orig_deg, m_):
        #let's find the best split of the graph
        BestQ = 0.0
        Q = 0.0
        while True:    
            self.CmtyGirvanNewmanStep(G)
            Q = self._GirvanNewmanGetModularity(G, Orig_deg, m_);
